...
export const s3Upload = (file) => {
  ...
  console.log('DEBUG s3Upload BUCKET:', BUCKET);
  const params = {
    Bucket: BUCKET,
...
export const s3ListFiles = async () => {
  ...
  const BUCKET = process.env.AWS_S3_BUCKET;
  console.log('DEBUG s3ListFiles BUCKET:', BUCKET);
...
export const s3Download = (key) => {
  ...
  const BUCKET = process.env.AWS_S3_BUCKET;
  console.log('DEBUG s3Download BUCKET:', BUCKET);
...
export const s3Delete = async (key) => {
  ...
  const BUCKET = process.env.AWS_S3_BUCKET;
  console.log('DEBUG s3Delete BUCKET:', BUCKET);
  const params = { Bucket: BUCKET, Key: key };
